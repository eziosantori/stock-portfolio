...
var iexUrl = process.env.IEXCLODUD_URL;
var iexTokenKey = "token=" + process.env.IEXCLODUD_TOKEN;
var getIexData = async (ticker, scope, additionalQs = "") => {
  try {
    const finalUrl = `${iexUrl}/stock/${ticker}/${scope}?${iexTokenKey}&${additionalQs}`;
    console.log(finalUrl);
    const res = await import_axios.default.get(finalUrl);
    return res.data;
  } catch (error) {
...
// functions/stocks/getIncomeStatment.ts
var getIncomeStatment = async (event, context, callback) => {
  const { symbol } = event.queryStringParameters;
  const res = await getIexData(symbol, "income", "period=annual");
...
var getIncomeStatment_default = getIncomeStatment;

// functions/income.ts
var handler = async (event, context, callback) => {
  if (event.httpMethod === "GET") {
    return await getIncomeStatment_default(event, context, callback);
  } else {
    return {
      statusCode: 405,
